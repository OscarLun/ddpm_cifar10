...
class FIDEvaluation:
    ...
    def fid_score(self, save_samples=False, n_samples=None):
        ...
        self.print_fn(
            f"Stacking Inception features for {n_samples} generated samples."
        )

        # Directory to save samples
        if save_samples:
            samples_dir = os.path.join(self.stats_dir, "test_samples")
        ...
        for i, batch in enumerate(tqdm(batches)):
            ...
            if save_samples:
                sample_path= os.path.join(
                    samples_dir, f"batch_{i}.npy"
                )
                np.save(sample_path, fake_samples.cpu().numpy())
