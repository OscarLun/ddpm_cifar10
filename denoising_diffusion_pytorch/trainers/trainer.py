...
#         steps_per_epoch = len(self.ds) // self.gradient_accumulate_every
#         total_epochs = (self.train_num_steps + steps_per_epoch - 1) // steps_per_epoch  # Round up

#         self.start_epoch = self.step
#         for epoch in range(self.start_epoch, total_epochs):
#             pbar = tqdm(self.dl, total=len(self.dl), disable=not accelerator.is_main_process)
...
#                     break # Stop training if the number of steps is reached
#                 data, _ = batch
#                 data = data.to(device)

#                 for _ in range(self.gradient_accumulate_every):
#                     # Only use inputs and not label
